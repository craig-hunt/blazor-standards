...
namespace BlazorStandards.Analyzers;
...
/// </summary>
/// <remarks>
/// An analyzer runs during compilation, so with warnings as errors a stray
/// literal fails the build rather than a test run. A literal may appear where it
/// is itself the named value: a const declaration, an enum member, an attribute
/// argument, a switch label, or a default parameter value.
///
/// This covers C# only. Razor markup compiles to generated code that analyzers
/// skip, so the Razor scan test covers component files. Two mechanisms, because
/// one cannot see both.
///
/// Component parameters matter here more than in a plain service project. A
/// Razor component exposes its inputs as properties, and a default value on one
/// is exactly where a stray literal hides.
/// </remarks>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class LiteralAnalyzer : DiagnosticAnalyzer
{
    public const string DiagnosticId = "BSD001";
    ...
    private const string Description =
        ...
        "test ids, routes, user-facing copy, configuration keys, log messages, and test data.";
    ...
    /// <summary>
    /// The empty string, zero, and one carry no meaning worth naming.
    /// </summary>
    ...
    /// <summary>
    /// A literal may appear where it is itself the named value.
    /// </summary>
